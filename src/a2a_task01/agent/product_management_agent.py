...
# #  MAF (pre) version 1.0.0b260210
# ChatAgent was replaced with RawAgent (class import, type annotation, and constructor usage).
# Constructor arg changed from chat_client= to client= when creating the agent.
# ai_function import was replaced with tool (renamed API surface in newer framework).
# ChatContent typing was removed/replaced with str in streaming and _get_agent_response, indicating the newer version no longer exposes ChatContent the same way.


# Old API: ChatAgent(chat_client=...)
# New API: RawAgent(client=...)
